...
function cp:create(x,y,size)
  local already_exists = not not self.canvas
    ...
    self.canvas:renderTo(
      function()
        ...
        for h=1,360 do
          ...
          local x1,y1=axisRot(0,lenOut,math.rad(h))
          local x2,y2=axisRot(0,lenIn,math.rad(h))
...
function cp:tri()
  ...
  local x1,y1=axisRot(0,lenIn,math.rad(self.ringDeg))
  local x2,y2=axisRot(0,lenIn,math.rad(self.ringDeg+120))
  local x3,y3=axisRot(0,lenIn,math.rad(self.ringDeg+240))
  local ver={
    {
            x1, y1,
            0, 0, 
  ...
  if love._version_minor >= 10 then
    mesh = love.graphics.newMesh(ver,"fan")
  else
...
function cp:update()
  local down = love.mouse.isDown(love._version_minor >= 10 and 2 or "r")
  if  down then
    local x, y = love.mouse.getPosition()
    local len = getDistance(x,y,self.cx,self.cy)
    if len>=0.75*self.size then
      self.ringDeg=math.deg(getRot(x,y,self.cx,self.cy))
      if len>self.size then 
        self.hx,self.hy=axisRot(0,1*self.size,math.rad(self.ringDeg))
      ...
      self.sx,self.sy=axisRot(0,self.oLen,math.rad(self.ringDeg+self.oDeg))
      self.sx=self.sx+self.x+self.size
      self.sy=self.sy+self.y+self.size
      local r,g,b,a=self:getColor(self.canvas2,self.sx,self.sy)
      self.sc={r,g,b}
      --self.oDeg=0
      self.ox,self.oy=axisRot(0,0.75*self.size,math.rad(self.ringDeg))
    ...
    elseif len<0.75*self.size then
      ...
      if a~=0 then
        ...
        self.sy=y
        self.sc={r,g,b}
        self.oDeg=math.deg(getRot(x,y,self.cx,self.cy))-self.ringDeg
        self.oLen=len
      else
        --self.ringDeg=math.deg(math.getRot(x,y,self.cx,self.cy))
        local diff=math.deg(getRot(self.ox,self.oy,self.cx,self.cy))-math.deg(getRot(x,y,self.cx,self.cy))
        self.ringDeg=self.ringDeg-diff
        self.hx,self.hy=axisRot(0,0.75*self.size,math.rad(self.ringDeg))
        self.hx=self.hx+self.x+self.size
        self.hy=self.hy+self.y+self.size
        self.sx,self.sy=axisRot(0,self.oLen,math.rad(self.ringDeg+self.oDeg))
        self.sx=self.sx+self.x+self.size
